    ...
    function renderDisclosureContent(wrap) {
        ...
        items.selectAll('input.value')
            ...
            .call(utilGetSetValue, function(d) {
                return typeof d.value === 'string' ? d.value : ''; // TODO - tags value
            })
            .attr('readonly', function(d) {
                return isReadOnly(d) || null;
            });
        ...
        let image = wrap.selectAll('.point-images img');

        if (image.empty()) {
            image = wrap.selectAll('.tag-list')
                .append('div')
                .attr('class', 'point-images')
                .append('img')
                .attr('src', () => {
						let zoneClass = _tags.zone?.split(';')[0] || '';
						let code = _tags.Code?.split(';')[0] || '';
                        const imageURL = `./images/Zone${zoneClass}/${code}.jpg`;
                        ...
                        imgTest.onerror = () => {
                            image.remove();
                            wrap.selectAll('.point-images').remove();
                        };
                    }
                )
                .attr('alt', "Obstacle image")
                .attr('width', "100%");
        }
